...
function setAuthUI({ authed, staff, email }) {
  ...
  if (authMsg) {
    ...
    else if (!staff) authMsg.textContent = `Signed in as ${email || "user"}, but not staff/admin. Set role='staff' or 'admin' in profiles.`;
...
async function requireStaff() {
  ...
  const role = res.profile?.role || "customer";
  const active = (res.profile?.is_active ?? true) === true;

  const staff = active && (role === "staff" || role === "admin"); // FIX: allow admin
...
function openUpdateModal(ds) {
  ...
  $("mWeight").value = ds.weight_lbs || "";  // FIX: now exists in HTML
  $("mCost").value = ds.cost_jmd || "";      // FIX: now exists in HTML
  $("mSendEmail").value = "no";
...
// Init
// --------------------
async function gateAndRenderOnce() {
...
// Subscribe ONCE (prevents double event storms if browser reloads oddly)
if (!window.__ADMIN_AUTH_SUB__) {
  window.__ADMIN_AUTH_SUB__ = supabase.auth.onAuthStateChange(() => {
    setTimeout(gateAndRenderOnce, 0);
  });
}
